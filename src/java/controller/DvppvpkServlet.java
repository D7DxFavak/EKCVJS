 ...
 * @author Favak
 */
@WebServlet(name = "DvppvpkServlet", loadOnStartup = 2,
urlPatterns = {"/dvppvpk"})
public class DvppvpkServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        zt.setIdKategorieZakladni(60);
        request.setAttribute("kategorieZakladniPopis", "dvppvpk");
        ...
        if (odkazNazev != null) {
            ...
            if (odkazNazev.equals("aktuality.jsp")) {
                if (!zt.getAktualniStranka().equals("dvppvpk?aktuality.jsp")) {
                    zt.vycistit("dvppvpk?aktuality.jsp");
                }
                Collection<WebAktuality> aktualitySeznam = aktualityFacade.findByKategorie(60, zt.getPocetStrana(), zt.getOffset());
                ...
                request.setAttribute("kategorieAktuality", kategorieFacade.findById(60).getWebKategorieZakladniPopis());

            } else if (odkazNazev.equals("galerie.jsp")) {
                if (!zt.getAktualniStranka().equals("ucitele?galerie.jsp")) {
                    zt.vycistit("ucitele?galerie.jsp");
                }
                Collection<WebGalerieFacade> galerieSeznam = webGalerieFacade.findByKategorie(kategorieFacade.findById(60), zt.getPocetStrana(), zt.getOffset());
                request.setAttribute("galerieSeznam", galerieSeznam);
                request.setAttribute("selectedCenter", odkazNazev);
                request.setAttribute("kategorieGalerie", kategorieFacade.findById(60).getWebKategorieZakladniPopis());

                long celkemZaznamu = webGalerieFacade.findPocetByKategorie(kategorieFacade.findById(60));
             ...
             * (!zt.getAktualniStranka().equals("dvppvpk?vseobecnepodminky.jsp"))
             * { zt.vycistit("dvppvpk?vseobecnepodminky.jsp"); } WebPanely wp =
             * panelyFacade.findByOdkazKategorie(odkazNazev,
             * kategorieFacade.findById(60)); Collection<WebPanelyData>
             ...
             * "WEB-INF/dvppvpk/vseobecnepodminky.jsp"); }
             */ else if (odkazNazev.equals("keStazeni.jsp")) {
                if (!zt.getAktualniStranka().equals("dvppvpk?keStazeni.jsp")) {
                    zt.vycistit("dvppvpk?keStazeni.jsp");
            ...
            } else if (odkazNazev.equals("dvppvpk.jsp")) {
                if (!zt.getAktualniStranka().equals("dvppvpk?dvppvpk.jsp")) {
                    zt.vycistit("dvppvpk?dvppvpk.jsp");
                }
                WebPanely wp = panelyFacade.findByOdkazKategorie(odkazNazev, kategorieFacade.findById(zt.getIdKategorieZakladni()));
                Collection<WebPanelyData> panelyData = webPanelyDataFacade.findByStranka(wp);
                request.setAttribute("panelyData", panelyData);
                request.setAttribute("selectedCenter", "WEB-INF/dvppvpk/zakladniInfo.jsp");
                request.setAttribute("idAktualitaDetail", zt.getIdAktualita());
                 ...
                 * if (request.getParameter("pocetStrana") != null) {
                 * zt.setPocetStrana(Integer.valueOf(request.getParameter("pocetStrana")).intValue());
                 * getServletContext().setAttribute("pocetStrana",
                 * zt.getPocetStrana()); } if (request.getParameter("offset") !=
                 * null) { int strana =
                 * Integer.valueOf(request.getParameter("offset")).intValue();
                 * if (strana == -1) { zt.setOffset(zt.getOffset() - 1); } else
                 * if (strana == -2) { zt.setOffset(zt.getOffset() + 1); } else
                 * { zt.setOffset(strana); }
                 * getServletContext().setAttribute("offset", zt.getOffset()); }
                 */
                ...
                request.setAttribute("selectedCenter", "WEB-INF/dvppvpk/" + odkazNazev);
                ...
                /*
                 * WebPanely wp = panelyFacade.findByOdkazKategorie(odkazNazev,
                 * kategorieFacade.findById(60)); Collection<WebPanelyData>
                 * panelyData = webPanelyDataFacade.findByStranka(wp);
                 * request.setAttribute("panelyData", panelyData);
                 * request.setAttribute("selectedCenter",
                 * "WEB-INF/dvpp/zakladniInfo.jsp");
                 * request.setAttribute("idAktualitaDetail", idAktualita);
                 */
            } else if (odkazNazev.equals("kontakty.jsp")) {
                if (!zt.getAktualniStranka().equals("dvppvpk?kontakty.jsp")) {
                    zt.vycistit("dvppvpk?kontakty.jsp");
                    zt.setIdOsOzHruba(80);
                }
                ...
                kontaktyOsOzHrube.add(webOsOblzarHrubeFacade.findById(80));
                ...
                if ((odkazNazev.contains("ulozisteData") == false) && (odkazNazev.contains("galerieUvodni") == false)
                    ...
                    if (!zt.getAktualniStranka().equals("dvppvpk?" + odkazNazev)) {
                        zt.vycistit("dvppvpk?" + odkazNazev);
                    }
                    WebPanely wp = panelyFacade.findByOdkazKategorie(odkazNazev, kategorieFacade.findById(zt.getIdKategorieZakladni()));
                    Collection<WebPanelyData> panelyData = webPanelyDataFacade.findByStranka(wp);
                    request.setAttribute("panelNazev", wp.getWebPanelyNazev());
