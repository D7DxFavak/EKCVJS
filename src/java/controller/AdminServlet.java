...
public class AdminServlet extends HttpServlet {
    ...
    @EJB
    private OsobyFacade osobyFacade;
    @EJB
    private PrihlaskyRozpisyFacade prihlaskyRozpisyFacade;
    private String userPath;
    private Osoby customer;
    private PrihlaskyRozpisy order;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userPath.equals("/admin/viewCustomers")) {
            customerList = osobyFacade.findAll();
            request.setAttribute("customerList", customerList);
        }

        // if viewOrders is requested
        if (userPath.equals("/admin/viewOrders")) {
            orderList = prihlaskyRozpisyFacade.findAll();
        ...
        if (userPath.equals("/admin/customerRecord")) {
            ...
            customer = osobyFacade.find(Integer.parseInt(customerId));
            request.setAttribute("customerRecord", customer);

            // get customer order details
            order = prihlaskyRozpisyFacade.findByOsoba(customer);
        ...
        if (userPath.equals("/admin/logout")) {            
            ...
            //response.sendRedirect("/ekcvjs2/admin/");

            userPath = "/logout.jsp";
            try {              
                request.getRequestDispatcher("/admin" + userPath).forward(request, response);                
            } catch (Exception ex) {
                ex.printStackTrace();
            }
            return;
