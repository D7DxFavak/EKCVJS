...
//Obecny SELECT pole datum z dotazu
 ...
 protected static synchronized String getFirstDayNextMonth() {
    java.util.Date datum = null;
    String navrat = "";
    try {
      ResultSet q = PripojeniDB.dotazS(
      "SELECT cast(to_char((date (COALESCE(CURRENT_DATE) + interval '1 month'))-1,'1.MM.YYYY') AS date)"
      );
      q.last();
      if(q.getRow() == 1)
      {
  ...
  protected static synchronized String getCurrentDatePlusInt(int interval) {
    java.util.Date datum = null;
    String navrat = "";
    try {
      ResultSet q = PripojeniDB.dotazS(
    ...
    protected static synchronized String getDatePlusInt(String datumIn, int interval) {
    java.util.Date datumOut = null;
    String navrat = "";
    try {
      ResultSet q = PripojeniDB.dotazS(
      "SELECT cast(' " + datumIn + "' AS date) + " + interval);
      q.last();
      if(q.getRow() == 1)
      {
      q.first();
      datumOut = q.getDate(1);
      if(!q.wasNull()) {
      navrat = df.format(datumOut);
      }//konec if q.wasNull
      } // konec if q.getRow() == 1
    } // konec try
    catch (Exception e)  {
...
//Metoda spoustejici prikaz - vraci 0 pri uspechu, jinak chybovy kod
  ...
  protected static synchronized int spustPrikaz(String prikaz)
  {
    ...
    {
            @SuppressWarnings("static-access")
      boolean b = PripojeniDB.dotazObecny(prikaz);
      navrat = 0;
    } // konec try
    catch (Exception e) {
